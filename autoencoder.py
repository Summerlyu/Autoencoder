...
momentum = 0.9
chosen_batch = 9
activation_threshold = -0.33  # washmachine
...
def net_training(iter_funcs, data):
    ...
    for epoch in itertools.count(1):
        ...
        result = dict(
            ...
            network_output = iter_funcs['network_output'](chosen_batch)[49],
            network_input = iter_funcs['network_input'](chosen_batch)[49],
            target = iter_funcs['target'](chosen_batch)[49]
            )

        for i in range(max_epochs):
            np.savetxt("result/network_output/"+str(i)+" network_output.txt", iter_funcs['network_output'](chosen_batch)[i], delimiter=",")
            np.savetxt("result/network_input/"+str(i)+" network_input.txt", iter_funcs['network_input'](chosen_batch)[i], delimiter=",")
            np.savetxt("result/target/"+str(i)+" target.txt", iter_funcs['target'](chosen_batch)[i], delimiter=",")
